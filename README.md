...
<!-- Added by: raphael, at: Wed Jul  3 09:50:47 CST 2019 -->
...
* 针对例如扫码支付、银行卡快捷支付等常规支付方式，将已成功支付的订单支付信息推送到指定海关。请注意如果订单是经由[2.自动支付相关接口](#2-自动支付相关接口)创建，则订单本身会自动进行申报，无需再通过本接口触发。或订单在创建支付请求时已指定了参数needDeclare为true，则订单再支付成功后也会自行申报，无需再单独调用本接口。
...
### 9.退款
#### 9.1 退款申请
请注意如果订单是经由[2.自动支付相关接口](#2-自动支付相关接口)提交至系统，则无法进行退款。退款将从商户的现金账户中扣除申请的退款额度，若现金账户余额不足将导致退款失败。
...
|:--|:--|:--|
|HG013|郑州海关||
|HG014|宁波海关| 宁波海关对接需要填写支付公司编码，若商户在易生支付入网请填写53 |
|HG016|重庆电子口岸（总署版）|若需要推送的海关不在列表中且其为总署版，可使用此代码进行推送|
|HG018|广州电子口岸3.0（总署版）||
|HG019|深圳电子口岸（总署版）||
|HG021|杭州电子口岸（总署版）||
|HG022|上海电子口岸（总署版）| 上海公服平台对接需要填写支付方式，支付方式代码请填写EASYPAY |
|HG026|天津电子口岸（总署版）||
|HG044|山东海关 ||
